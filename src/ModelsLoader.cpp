...
Anni::ModelLoader::Node::Node() : IRenderable()
{
}
...
void Anni::ModelLoader::Node::DrawStaticToContext(const glm::mat4& top_matrix, DrawContext& ctx)
{
	// draw children
	for ( const auto& c : children )
	{
		c->DrawStaticToContext(top_matrix, ctx);
	}
}
...
void Anni::ModelLoader::MeshNode::DrawStaticToContext(const glm::mat4& top_matrix, DrawContext& ctx)
{
	...
	// recurse down(call DrawStaticToContext() as Node(i.e. as base class's virtual function), which iterate the Node's all children Nodes.)
	Node::DrawStaticToContext(top_matrix, ctx);
}

Anni::ModelLoader::LoadingResult::LoadingResult(std::filesystem::path file_path) : m_file_path(std::move(file_path))
{

}

std::unique_ptr<Anni::ModelLoader::LoadingResult> Anni::ModelLoader::LoadFromFile(const std::filesystem::path file_path)
{
	...
	std::unique_ptr<LoadingResult> loading_result = std::make_unique<LoadingResult>(file_path);
...
static void Anni::ModelLoader::LoadGltf(const std::filesystem::path& file_path, fastgltf::Parser& gltf_parser, std::unique_ptr<LoadingResult>& loading_result)
{
...
static void Anni::ModelLoader::LoadRawGltf(fastgltf::GltfDataBuffer& data, fastgltf::Asset& gltf_asset, const std::filesystem::path& file_path, fastgltf::Parser& gltf_parser)
{
	//> LOAD_RAW GLTF RAW FILE LOADING
	constexpr auto gltf_loading_options = fastgltf::Options::DontRequireValidAssetMember | fastgltf::Options::AllowDouble | fastgltf::Options::LoadGLBBuffers | fastgltf::Options::LoadExternalBuffers;
	...
	const fastgltf::GltfType gltf_type = fastgltf::determineGltfFileType(&data);

	if ( fastgltf::GltfType::glTF == gltf_type )
	{
		auto load = gltf_parser.loadGLTF(&data, file_path.parent_path(),
										 gltf_loading_options);
		if ( load )
		{
	...
	{
		auto load = gltf_parser.loadBinaryGLTF(&data, file_path.parent_path(),
											   gltf_loading_options);
		if ( load )
		{
			gltf_asset = std::move(load.get());
...
static void Anni::ModelLoader::LoadSamplers(const fastgltf::Asset& gltf_asset, std::unique_ptr<LoadingResult>& loading_result)
{
...
static Anni::ModelLoader::LoadedSampler::AddressMode Anni::ModelLoader::ExtractAddressMode(const fastgltf::Wrap warp)
{
...
static Anni::ModelLoader::LoadedSampler::SamplerType Anni::ModelLoader::ExtractMagSamplerType(const fastgltf::Filter filter)
{
	//read the spec, found out this is the way they define this
	switch ( filter )
	{
...
static Anni::ModelLoader::LoadedSampler::SamplerType Anni::ModelLoader::ExtractMinSamplerType(const fastgltf::Filter filter)
{
	switch ( filter )
	{
		case fastgltf::Filter::Nearest:
...
static Anni::ModelLoader::LoadedSampler::SamplerType Anni::ModelLoader::ExtractMipMapSamplerType(const fastgltf::Filter filter)
{
...
static void Anni::ModelLoader::LoadTextureImages(const fastgltf::Asset& gltf_asset, const std::filesystem::path& file_path, std::unique_ptr<LoadingResult>& loading_result)
{
...
static void Anni::ModelLoader::LoadMaterials(const fastgltf::Asset& gltf_asset, const std::filesystem::path& file_path, std::unique_ptr<LoadingResult>& loading_result)
{
...
static void Anni::ModelLoader::LoadMeshes(const fastgltf::Asset& gltf_asset, std::unique_ptr<LoadingResult>& loading_result)
{
	...
	for ( auto [mesh_index, mesh] : std::ranges::views::enumerate(gltf_asset.meshes) )
	{
		...
		for ( auto&& p : mesh.primitives )
		{
			...
			homo_mat_tris.count = static_cast< uint32_t >(gltf_asset.accessors[p.indicesAccessor.value()].count);

			size_t initial_vtx = vertices.size();

			// load indexes
			{
				const fastgltf::Accessor& indexaccessor = gltf_asset.accessors[p.indicesAccessor.value()];
			...
			{
				const fastgltf::Accessor& posAccessor = gltf_asset.accessors[p.findAttribute("POSITION")->second];
			...
			{
				fastgltf::iterateAccessorWithIndex<glm::vec3>(
					gltf_asset, gltf_asset.accessors[normals->second],
			...
			if ( uv != p.attributes.end() )
			{
				fastgltf::iterateAccessorWithIndex<glm::vec2>(
					gltf_asset, gltf_asset.accessors[uv->second],
			...
			if ( colors != p.attributes.end() )
			{
				fastgltf::iterateAccessorWithIndex<glm::vec4>(
					gltf_asset, gltf_asset.accessors[colors->second],
			...
			// load tangent
			const auto tangent = p.findAttribute("TANGENT");
			if ( tangent != p.attributes.end() )
			{
				fastgltf::iterateAccessorWithIndex<glm::vec3>(
					gltf_asset, gltf_asset.accessors[tangent->second],
					[&](glm::vec3 v, size_t index)
					{
			...
			{
				homo_mat_tris.material_index = p.materialIndex.value();
			}
			else
			{
				//SPD log here
...
static void Anni::ModelLoader::LoadSceneNodes(const fastgltf::Asset& gltf_asset, std::unique_ptr<LoadingResult>& loading_result)
{
	...
	for ( auto [node_index, node] : std::ranges::views::enumerate(gltf_asset.nodes) )
	{
		...
		{
			new_node = std::make_shared<MeshNode>(&loading_result->m_mesh_assets[*node.meshIndex]);
		}
		else
		{
			//TODO spd log here
